...
test("preview state broadcasting records the page displayed for the session agent", async () => {
  clearSessionUiSyncState();
  const service = createService({
    projectService: {
      async createRuntime() {
  ...
  const invalid = await service.broadcastSessionPreviewState("session-1", {
    originId: "tab-1",
    projectSlug: "beepollen",
