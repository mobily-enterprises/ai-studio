...
test("vibe64 runtime exposes composer menu templates and current workflow actions", async () => {
  await withTemporaryRoot(async (targetRoot) => {
    ...
    assert.deepEqual(deslopChanges?.groupPath, ["Code", "Deslop"]);
    assert.equal(deslopChanges?.label, "Only changes");
    ...
    assert.deepEqual(deslopCodebase?.groupPath, ["Code", "Deslop"]);
    assert.equal(deslopCodebase?.label, "Whole codebase");
    ...
    assert.deepEqual(checkUiChanges?.groupPath, ["Code", "Check UI"]);
    assert.equal(checkUiChanges?.label, "Only changes");
    ...
    assert.deepEqual(checkUiCodebase?.groupPath, ["Code", "Check UI"]);
    assert.equal(checkUiCodebase?.label, "Whole codebase");
    ...
    assert.equal(createHandover?.group, "Info");
