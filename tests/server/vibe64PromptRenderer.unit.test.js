...
test("session briefing keeps the primary preview canonical while permitting explicit reference apps", () => {
  ...
  assert.match(briefing, /vibe64-preview browser identity default/u);
  assert.match(briefing, /vibe64-preview browser identity <configured-name>/u);
  assert.match(briefing, /never infer one from the signed-in Vibe64 account/u);
  ...
  assert.match(briefing, /vibe64-playwright --identity <configured-name> test/u);
  assert.match(briefing, /Do not guess or pass a raw application identifier/u);
  ...
  assert.match(briefing, /project's default managed app identity/u);
  assert.doesNotMatch(briefing, /current Vibe64 viewer/u);
