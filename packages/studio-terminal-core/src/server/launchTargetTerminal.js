...
function commandWithHttpReadiness({
  ...
  return [
    ...
    "  vibe64_readiness_pid=$!",
    "  set +e",
    "  wait -n \"$vibe64_launch_pid\" \"$vibe64_readiness_pid\"",
    "  vibe64_first_status=$?",
    "  set -e",
    "  if kill -0 \"$vibe64_readiness_pid\" 2>/dev/null; then",
    "    kill \"$vibe64_readiness_pid\" 2>/dev/null || true",
    "    wait \"$vibe64_readiness_pid\" 2>/dev/null || true",
    "    vibe64_readiness_pid=",
    "    if [ \"$vibe64_first_status\" -eq 0 ]; then",
    "      echo '[studio] Launch target exited before it became ready.' >&2",
    "      exit 1",
    "    fi",
    "    exit \"$vibe64_first_status\"",
    "  fi",
    "  vibe64_readiness_pid=",
    "  if [ \"$vibe64_first_status\" -ne 0 ]; then",
    "    kill \"$vibe64_launch_pid\" 2>/dev/null || true",
    "    wait \"$vibe64_launch_pid\" 2>/dev/null || true",
    "    exit \"$vibe64_first_status\"",
    "  fi",
