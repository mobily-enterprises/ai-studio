...
function jskitSelfTargetRootConfig({
  ...
  return {
    ...
    previewProxyPortRange: previewProxy.portRange,
...
function jskitDevBackendPortStart(frontendPort = DEFAULT_WEB_LAUNCH_TARGET_PORT) {
  const normalizedFrontendPort = normalizeWebLaunchPort(frontendPort);
  return normalizedFrontendPort < 65535
    ? normalizedFrontendPort + 1
    : DEFAULT_WEB_LAUNCH_TARGET_PORT + 1;
}
...
async function resolveBuiltLaunchConfig(worktreePath, {
  ...
  const [configuredBuiltCommand, selfTarget, migrationCommand, portValue] = await Promise.all([
    ...
    readOptionalConfigFile(worktreePath, BUILT_LAUNCH_PORT_CONFIG, String(DEFAULT_WEB_LAUNCH_TARGET_PORT))
  ]);
  if (configuredBuiltCommand) {
    return {
      ...
      migrationCommand,
      preferredPort: normalizeWebLaunchPort(portValue),
...
function jskitFrontendRestartControlLines({
  ...
  return [
    ...
    "vibe64_jskit_read_server_fingerprint() {",
    `  ${serverFingerprintCommand}`,
    "}",
    ...
    "}",
    "vibe64_jskit_record_server_fingerprint() {",
    "  vibe64_jskit_server_fingerprint=\"$(vibe64_jskit_read_server_fingerprint || true)\"",
    "}",
    "vibe64_jskit_server_files_changed() {",
    "  vibe64_jskit_next_server_fingerprint=\"$(vibe64_jskit_read_server_fingerprint || true)\"",
...
function jskitServerFingerprintCommand() {
  const script = [
    ...
    launchRestartRulesMatcherSource(),
    "const matchesRestartPath = vibe64LaunchRestartRulesMatcher({ include, exclude });",
    "const excludesRestartPath = vibe64LaunchRestartRulesMatcher({ include: exclude });",
    ...
    "  return excludesRestartPath(relativePath);",
    "}",
    ...
    "      continue;",
    "    }",
    "    if (!dirent.isFile() || !matchesRestartPath(relativePath)) {",
    "      continue;",
    "    }",
    "    try {",
    ...
    const spec = await createVibe64WebLaunchTargetTerminalSpec({
      ...
      resolveLaunch: async ({
        ...
        const selfTarget = jskitSelfTargetRootConfig({
          ...
          systemRoot: nestedStudioSessionRuntimeRoot({
            directoryName: "self-target-system-root",
            session,
            worktreePath: launchWorktreePath
          })
        });
