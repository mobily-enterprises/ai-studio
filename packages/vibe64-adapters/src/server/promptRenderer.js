...
function managedPreviewPolicyInstruction() {
  return [
    ...
    "- Codex's internal managed browser has its own application session, separate from the user's visible Preview. Use `vibe64-preview browser identity default` for the project's default managed app identity, `vibe64-preview browser identity <configured-name>` for another named app identity such as admin or user, or `vibe64-preview browser identity guest` to sign it out. Run `vibe64-preview status` to see the configured names. Vibe64 resolves each name to the app-owned identifier configured in project state; never infer one from the signed-in Vibe64 account. These commands never change the user's visible Preview.",
    ...
    "- Keep ordinary portable Playwright tests in the project with its normal `@playwright/test` dev dependency and a `PLAYWRIGHT_BASE_URL` override. Inside Vibe64, immediately run them through `vibe64-playwright test [arguments]` or `vibe64-playwright npm-run <script> [-- arguments]`. These commands automatically ensure the current managed preview, supply its agent origin as `PLAYWRIGHT_BASE_URL`, pair the project test package with the exact managed browser version, and authenticate identity-aware application tests through temporary managed storage state. They use the project's default managed app identity; select another configured name with `vibe64-playwright --identity <configured-name> test [arguments]` or the equivalent `npm-run` command. Run `vibe64-preview status` to see the available names. Do not guess or pass a raw application identifier. Do not call development-auth endpoints, copy or compare bypass secrets, configure storage state manually, or register throwaway users merely to authenticate a feature test. Exercise sign-in or registration only when that authentication behavior itself is under test. Do not inspect or hard-code managed ports, add Vibe64 URL-discovery helpers to the project, or pass the managed preview URL manually. Never install browser payloads in the project or user cache.",
...
function promptSessionBriefing(contextInput = {}) {
  ...
  return [
    ...
    "Committed source contract files are `vibe64.project.json`, `vibe64.runtime-lock.json`, `vibe64.system.json`, optional `.vibe64/bin`, `.vibe64/launcher`, `.vibe64/scripts`, `.vibe64/prompts`, and `.vibe64/project-knowledge`.",
    "Vibe64-owned project storage includes `runtime-config/*`, sessions, runtime, the disposable GitHub mirror, the durable canonical repository, terminal/provider state, and other managed workspace state.",
    ...
    "Do not commit or rewrite Vibe64-owned project storage unless the current workflow explicitly instructs you to inspect or report that storage.",
    "",
    "Code index policy:",
    codeIndexPolicy,
    "",
